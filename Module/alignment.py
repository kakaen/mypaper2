...
class AlignedConv2d(nn.Module):
    def __init__(self, inc, outc=1, kernel_size=3, padding=1, stride=1, bias=None, modulation=False):
        ...
        #这里修改3为6
        head = [nn.Conv2d(6, 32, kernel_size=5, padding=2, stride=1), nn.LeakyReLU(0.2, inplace=True), ResBlock(in_channels=32, out_channels=32), nn.LeakyReLU(0.2, inplace=True)]
    ...
    def forward(self, x, query,ref):
        query = F.interpolate(query, scale_factor=1, mode='bicubic')
        ref = self.conv1(ref)
        query = self.conv1(query)
        #这里是生成Local Alignment的conv
